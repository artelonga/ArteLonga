...
The public page `artelonga.com.br/analytics/` (noindex) consumes these
endpoints — **no auth**, aggregated read-only, no PII. They all return
JSON. CORS open for `https://artelonga.com.br`.

### `GET /api/v1/analytics/public/summary?days=30`

Returns aggregates for the requested range (1, 7, 30 or 90 days).

**Actual shape served today** (verified 2026-06-05 — note `as_of`/`window_days`,
not `range`):

```ts
type SummaryResponse = {
  as_of: string,                                       // ISO datetime of the computation
  window_days: number,                                 // requested window (= days)
  views: number,                                       // total page_view
  events_total: number,                                // all events in range
  visitors: number,                                    // unique vid
  returning: number,                                   // visitors that had a vid before the range
  sessions: number,                                    // unique sid
  session_avg_ms: number,                              // average duration (ms) — ⚠ current bug: returns ~4ms
  countries: number,                                   // distinct
  cities: number,
  timeseries: Array<{ bucket: string, count: number }>, // bucket = ISO date (or hour if days=1)
...
Bucketing: `days=1` → hour, otherwise → day. Limit top_pages = 30, geo = 50.

**Planned parameters (still ignored by the backend — the front end already feature-detects):**
...
> ASN/datacenter filter is recommended on the backend before counting them as a visitor.

### `GET /api/v1/analytics/public/recent?limit=50`

Recent stream — the last N events (max 200). Useful for a "live tail" on the dashboard.
...
No `vid`, `sid`, `ip_hash`, `ua_brand`, `props`. Only what the public is allowed to see.

### Privacy

- None of the endpoints expose `vid`, `sid`, `ip_hash`, UTM, props, experiments or
  any field that could identify an individual visitor.
- Events with `name = js_error` or `js_promise_rejection` are kept out of
  `recent` (noise + risk of the stack leaking info).
- HTTP cache: `Cache-Control: public, max-age=60` is reasonable (the dashboard
  doesn't need to be absolutely real-time).
- Recommended per-IP rate-limit: 30 req/min.
