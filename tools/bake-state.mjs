#!/usr/bin/env node
// Gera docs/STATE.md com data, última release, commits recentes e backlog abertos.
// Uso: node tools/bake-state.mjs  (ou npm run bake-state)
// Determinístico: rodar no mesmo dia produz o mesmo output.
...
function extractLastRelease() {
  ...
  return result.join("\n").trim() || "_(nenhuma release encontrada)_";
}

function gitLog30(grep) {
  try {
    const out = execSync(
      `git -C "${ROOT}" log --since="30 days ago" --grep="${grep}" --pretty=format:"- %s (%h)"`,
      { encoding: "utf8", stdio: ["pipe", "pipe", "pipe"] }
    ).trim();
    return out || "_(nenhum no período)_";
  } catch {
    return "_(nenhum no período)_";
  }
}

function gitTotalCommits() {
  try {
    return execSync(`git -C "${ROOT}" rev-list --count HEAD`, {
      encoding: "utf8",
    }).trim();
  } catch {
    return "?";
...
function openBacklog() {
  const workDir = join(ROOT, "work/artelonga");
  const files = readdirSync(workDir)
    .filter((f) => /^AL-\d+\.md$/.test(f))
    .sort((a, b) => {
  ...
  for (const file of files) {
    ...
    if (status === "todo" || status === "in_progress") {
      ...
      const label = status === "in_progress" ? "em andamento" : "aberto";
      rows.push(`| ${id} | ${label} | ${title} |`);
    }
  }

  if (rows.length === 0) return "| — | — | Nenhum aberto no momento |";
  return rows.join("\n");
}

// --- Build ---
const date = today();
const lastRelease = extractLastRelease();
const recentFeats = gitLog30("^feat");
const recentFixes = gitLog30("^fix");
const totalCommits = gitTotalCommits();
const backlog = openBacklog();

const STATE = `# Estado do projeto · ${date}
...
console.log(`docs/STATE.md gerado: ${date}`);
